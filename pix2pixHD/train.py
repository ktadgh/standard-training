...
from models.networks import DistillLoss, GlobalGenerator




teacher = GlobalGenerator(None,None, None, teacher = True)
dct = torch.load('/home/ubuntu/transformer-distillation/200_net_G_hdit.pth')
dct = {
    key.replace('model.', ''): value for key, value in dct.items()
}
teacher.model.load_state_dict(dct)
...
run = Run(experiment = 'distillation-test1')
...
distill_loss = DistillLoss(teacher.model, model.module.netG.model)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        dloss = distill_loss(data['label'])
        # calculate final loss scalar
        loss_D = (loss_dict['D_fake'] + loss_dict['D_real']) * 0.5
        loss_G = loss_dict['G_GAN'] + loss_dict.get('G_GAN_Feat',0) + loss_dict.get('G_VGG',0) + dloss
        ...
        run.track(dloss, name = 'distillation loss')
