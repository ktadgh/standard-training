...
## Notes on installation and debugging
For installing with CUDA 12.0 I wasn't able to compile the modules in the `/networks` folder using the bash script - one of the issues found that changing the line:
```
cxx_args = ['-std=c++11']
```
to 
...
cxx_args = ['-std=c++14']
```
solved it (for a different version of CUDA).
```
cxx_args = ['-std=c++17']
```
worked for CUDA 12.0.
Also, installing each of the modules individually with ```pip install --no-build-isolation -e .```, using a venv with the requirements installed, worked better than the method in the bash file, which is deprecated.
Lastly on running the model I had the error  `CUDA call failed` - this was fixed by commenting out the test in the file `correlation_cuda.cc` as suggested [here](https://github.com/NVIDIA/flownet2-pytorch/issues/114)
